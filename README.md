...
or use a specic tag, for example the v11.0.9g release:
```docker run -p 1360:1360 -d mimersql/mimersql_v11.0:v11.0.9g-rest```
...
From Mimer SQL 11.1, the superuser is no longer required to be named SYSADM and can be set with ```-e MIMER_SUPERUSER=<name>```. If not specified it defaults to SYSADM. This has no effect on 11.0 images, where the superuser is always SYSADM.
...
Calls like `/log_sql` and `/update_sysadm_pass` that take a Mimer SQL superuser password operate against whichever superuser is configured for the database — SYSADM by default, or whatever `-e MIMER_SUPERUSER=<name>` was set to (see above).
